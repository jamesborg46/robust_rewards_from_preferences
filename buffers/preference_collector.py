...
class Segment(TimeStepBatch):
    ...
    @property
    def total_gt_reward(self):
        return sum(self.env_infos['gt_reward'])
...
class PreferenceCollector(abc.ABC):
    ...
    def get_labelled_preferences(self):
        left = np.array(
            [self._get_model_input_from_segment(comp['left']) for comp
             in self._comparisons if comp['label'] in [0, 1]]
        )

        right = np.array(
            [self._get_model_input_from_segment(comp['right']) for comp
             in self._comparisons if comp['label'] in [0, 1]]
        )

        prefs = np.array(
            [comp['label'] for comp in self._comparisons
             if comp['label'] in [0, 1]]
        )
...
class SyntheticPreferenceCollector(PreferenceCollector):
    ...
    def _add_synthetic_label(comparison):
        ...
        left_has_more_rew = (
            left_seg.total_gt_reward > right_seg.total_gt_reward
        )
...
# TODO: Fix up this implementation
...
#         comparison_id = self._create_comparison_in_webapp(left_seg,
#                                                           right_seg)
#         comparison = {
#             "left": left_seg,
#             "right": right_seg,
...
#         """Creates a comparison DB object.
#         Returns the db_id of the comparison"""
#         from human_feedback_api import Comparison

#         comparison_uuid = str(uuid.uuid4())
#         comparison = Comparison(
#             experiment_name=self.experiment_name,
#             media_url_1=self.convert_segment_to_media_url(comparison_uuid,
#             'left', left_seg),
#             media_url_2=self.convert_segment_to_media_url(comparison_uuid,
#             'right', right_seg),
...
class LabelAnnealer(object):
    ...
    def __init__(self, number_epochs, final_labels, pretrain_labels):
    ...
    def n_desired_labels(self, itr):
        ...
        # Decay from 1 to 0
        exp_decay_frac = 0.01 ** (itr / self._number_epochs)
